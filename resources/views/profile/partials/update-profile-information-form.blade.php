<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Perbarui informasi profil akun, alamat, dan email Anda.') }}
        </p>
    </header>
...
{{-- Script untuk Dynamic Dropdown Wilayah --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {

        // UNTUK EDIT PROFILE
        // ID di HTML: id="province_id", id="city_id", id="district_id"

        window.initWilayahDropdown(
            'province_id',
            'regency_id',
            'district_id', // Ada kecamatan disini
            {
                prov: "{{ old('province_id', $user->province_id) }}",
                city: "{{ old('regency_id', $user->regency_id) }}",
                dist: "{{ old('district_id', $user->district_id) }}"
            }
        );
    });
</script>
